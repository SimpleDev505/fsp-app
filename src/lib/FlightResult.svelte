<script lang="ts">
  ...
  //Staff View
  function MarkFlight(mark: boolean) {
    if (!$SelectedFlightOffer && mark) {
      SelectedFlightOffer.set(flight);
  ...
  <div class="main-info" onmousedown={() => (showDetails = !showDetails)}>
    ...
    <div class="price-col">
      ...
      {:else if !markedforreview}
        <button class="book-btn" onclick={() => MarkFlight(true)}>Mark</button>
      {:else}
        <button
          style="outline: 2px solid var(--yellow);"
          class="book-btn"
          onclick={() => MarkFlight(false)}>UnMark</button
        >
      {/if}
    </div>
  </div>
