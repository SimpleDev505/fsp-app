...
app.post("/api/book-ticket", async (req, res) => {
  try {
    ...
    // Create and save new ticket document
...
app.post("/api/tickets/assign-seats", async (req, res) => {
  const { ticketNumber, seatAssignments } = req.body;
  // seatAssignments = ['12A', '14C', ...] same length as passengers

  try {
    const ticket = await Ticket.findOne({ ticketNumber });
    if (!ticket) return res.status(404).json({ message: "Ticket not found" });

    if (seatAssignments.length !== ticket.passengers.length) {
      return res.status(400).json({ message: "Passenger count mismatch" });
    }

    // Assign seats to each passenger directly
    ticket.passengers.forEach((p, i) => {
      p.seatNumber = seatAssignments[i];
    });

    await ticket.save();
    return res
      .status(200)
      .json({ message: "Seats assigned successfully", ticket });
  } catch (err) {
...
app.post("/api/tickets/search", async (req, res) => {
  ...
  try {
    ...
    // 2. Find all other tickets on same flight (airlineCode + flightNumber + date)
    const { airlineCode, flightNumber } = currentTicket.flight;

    const otherTickets = await Ticket.find({
      "flight.airlineCode": airlineCode,
      "flight.flightNumber": flightNumber,
      "flight.from.dateTime": currentTicket.flight.from.dateTime,
      ticketNumber: { $ne: currentTicket.ticketNumber }, // exclude current ticket
    });

    // Flatten all passengers from those tickets
    const otherPassengersOnSameFlight = otherTickets.flatMap((ticket) =>
      ticket.passengers.map((p) => ({
        ...
        seat: ticket.seats?.[ticket.passengers.indexOf(p)] ?? null,
      }))
    );
