...
app.post("/api/tickets/search", async (req, res) => {
  ...
  try {
    ...
    // 2. Extract flight identity (use airlineCode + flightNumber + dateTime)
    const { airlineCode, flightNumber } = currentTicket.flight;
    const flightDate = currentTicket.flight.from.dateTime;

    // 3. Fetch other tickets on same flight (excluding current ticket)
    ...
    // 4. Flatten passengers from other tickets
    ...
    return res.status(200).json({
      currentTicket, // includes passengers with seatNumber
