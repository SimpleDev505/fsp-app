<script lang="ts">
  const type = $prop<"DELAY" | "RESCHEDULED" | "CANCELED">();
  const flight = $prop<FlightOffer>();
  const onClose = $prop<() => void>();

  const delayHours = $state<number | null>(null);
  const delayMins = $state<number | null>(null);
  const delayReason = $state("");
  const delayError = $state<StatusMessage>({ message: null, type: "ERROR" });

  const cancelReason = $state("");
  const cancelError = $state<StatusMessage>({ message: null, type: "ERROR" });
...
<div class="overlay">
  ...
  <div class="actions">
    ...
    >
    <Button variant="outline" on:click={onClose}>Back</Button>
  </div>
</div>
