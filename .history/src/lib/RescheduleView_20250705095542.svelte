<script lang="ts">
  ...
  import { DatePickerOpen, SelectedDate } from "./Store/store";
  ...
  $effect(() => {
    if ($DatePickerOpen === true) {
      gsap.to(".reason-group", {
        opacity: 0,
      });
    } else if ($DatePickerOpen === false) {
      gsap.to(".reason-group", {
        opacity: 1,
      });
    }
  ...
  const validateDelay = (): boolean => {
    ...
    if (!reason.trim()) {
      profile_err = { message: "Please enter Reason", type: "ERROR" };
      setTimeout(() => {
        profile_err.message = null;
      }, 4000);
      return false;
    ...
    switch (type) {
      case "DELAY":
        ...
        if (isValid) {
          ...
          const originalTime = new Date(flight.lastTicketingDate);
  ...
  async function ScheduleFlight(
    ...
    try {
      ...
      return response.data.message || "Flight scheduled successfully";
    } catch (error) {
  ...
  async function LoadLatestScheduledFlights(): Promise<void> {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/schedule/latest`
      );
      schFlights.set(response.data.data);
    } catch (error) {
      if (axios.isAxiosError(error)) {
        throw new Error(error.response?.data?.error || error.message);
      }
