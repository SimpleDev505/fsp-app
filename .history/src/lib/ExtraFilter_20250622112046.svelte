<script lang="ts">
  ...
  function updateFilter(filterKey: keyof typeof DEFAULT_FILTERS, value: any) {
    ...
    ExtraFiltered_Results.set(
      $FlightSearchResults.filter((flight) => {
        switch (filterKey) {
          case "popularfilters":
            if (value === "1") {
              return flight.itineraries[0].segments.length === 1;
            } else if (value === "2") {
              return flight.itineraries[0].segments.length >= 3;
            } else if (value === "Air India" || value === "SpiceJet") {
              return (
                flight.itineraries[0].segments[0].carrierCode ===
                (value === "Air India" ? "AI" : "SG")
              );
            } else {
              return true;
            }
          ...
          case "pricerange":
            return parseInt(flight.price.total) <= value;

          case "stops":
            const stops = flight.itineraries[0].segments.length - 1;
            if (value === "1") return stops === 1;
            if (value === "2") return stops >= 2;
            if (value === "none") return stops === 0;
            return true;

          case "departime":
            const hour = parseInt(
              flight.itineraries[0].segments[0].departure.at.slice(11, 13)
            );
            if (value === "Before 6Am") return hour < 6;
            if (value === "6Am to 12Pm") return hour >= 6 && hour < 12;
            if (value === "12Pm to 6Pm") return hour >= 12 && hour < 18;
            if (value === "After 6Pm") return hour >= 18;
            return true;

          case "duration_hrs":
            const match = flight.itineraries[0].duration.match(/PT(\d+)H/);
            const durHours = match ? parseInt(match[1]) : 0;
            return durHours <= value;

          case "airlines":
            return (
              flight.itineraries[0].segments[0].carrierCode ===
              getCarrierCodeFromName(value)
            );
