...
<div class="flight-card">
  <div class="main-info" on:click={toggleDetails}>
    ...
    <!-- <div class="duration">
      <p>
        <span
          style="width: 100%; display: flex; flex-direction: row; justify-content: center;align-items:center; column-gap: 5px;"
        >
    ...
    {#if flight?.itineraries[0]?.segments?.length > 1}
      <div class="layover-card">
        <h4>
          <span class="material-symbols-outlined">connecting_airports</span> Layover
          Details
        </h4>
        {#each flight.itineraries[0].segments as seg, i (seg.id)}
          {#if i < flight.itineraries[0].segments.length - 1}
            <div class="layover-segment">
              ...
              <p>
                Arrival: {seg.arrival.at?.slice(11, 16)} → Next Departure: {flight.itineraries[0].segments[
                  i + 1
                ].departure.at?.slice(11, 16)}
              </p>
              <p>
                Layover Duration: new Date( flight.itineraries[0].segments[i +
                1].departure.at ).getTime() - new
                Date(seg.arrival.at).getTime()} ms
              </p>
            </div>
          {/if}
        {/each}
