<script lang="ts">
  ...
  let DropDownIcon = $derived(
    items ? (items[0].icon ? items[0].icon : "") : "Title"
  );
...
<div
  class="dropdown-menu"
  style="position: relative; background-color: transparent; color: black; border: 2px solid var(--grey-light); border-radius: 4px;z-index: 1000;"
>
  <button
  ...
  {#if DropDown}
    <div
      style="position: absolute; top: 100%;width: fit-content; background: white; border: 2px solid var(--grey-light); border-radius: 4px; z-index: 100000; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"
    >
      {#each items as item}
        <div
          role="button"
