<script lang="ts">
  ...
  // Helper: Convert "24'Jun'25" to "2025-06-24"
  ...
  async function FetchFlights() {
    const API_KEY = import.meta.env.FLIGHT_API_KEY;
    const filter = get(Current_SearchFilter);

    const { from, to, dep } = filter.timings[0]; // support One Way for now
    ...
    try {
      ...
      FlightSearchResults.set(flights);
