<script lang="ts">
  ...
  import { fetchLatestScheduledFlights, schFlights } from "./Store/schedule";
          ...
          <div class="flight-items">
            {#each $schFlights as flight}
              <div class="flight-card">
                <p>
                  {flight.flight.itineraries[0].segments[0].departure} - {flight
                    .flight.itineraries[0].segments[0].arrival}
                </p>
                <small
                  >{flight.flight.itineraries[0].segments[0].aircraft.code} | {flight
                    .flight.itineraries[0].segments[0].duration}</small
                >
                <span class="status-tag green">{flight.scheduledType}</span>
              </div>
            {/each}
          </div>
        </div>
