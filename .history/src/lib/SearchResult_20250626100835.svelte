<script lang="ts">
  ...
  onMount(() => {
    ...
    gsap.from(".flight-card", {
      ...
      duration: 0.6, // Each individual animation takes 0.6 seconds
      stagger: 0.15, // Delay between the start of each element's animation (0.15 seconds)
      ease: "power2.out", // A smooth easing function
      // onComplete: () => console.log("All flight cards animated in!"), // Optional: callback when the *entire* stagger is done
    });
  });
</script>
