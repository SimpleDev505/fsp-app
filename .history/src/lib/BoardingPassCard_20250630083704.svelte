  ...
  if (seats && seats.length > 0) {
    ...
    const alreadyAssigned = Object.entries(seatAssignments).find(
      ([existingPassenger, seatNumber]) =>
        seatNumber === lastSeat.seatNumber && existingPassenger !== passengerName
    );
    if (alreadyAssigned) {
      assgn_err.message = "`Seat ${lastSeat.seatNumber} is already assigned to ${alreadyAssigned[0]}`";
      assgn_err.type = "WARN";
      setTimeout(() => {
    ...
    seatAssignments[passengerName] = lastSeat.seatNumber;
    assgn_err.message =`Assigned seat ${lastSeat.seatNumber} to ${passengerName}`
    console.log(`Assigned seat ${lastSeat.seatNumber} to ${passengerName}`);
  }
}
          ...
          {#if ticketdetail && !$fetchingTicket}
            ...
            {#each ticketdetail.passengers as p, pindex}
              <div class="passenger-row">
                ...
                <span>{p.seatnumber ?? "Not Assigned"}</span>
                <span><button onclick={() => }>Assign</button></span>
