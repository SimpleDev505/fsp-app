<script lang="ts">
  ...
  // 1. Create a 2D grid based on coordinates
  const seatGrid: (Seat | null)[][] = [];

  // 2. Find max row and column from coordinates
  const maxRow = Math.max(
    ...($SeatMapData?.decks?.[0]?.seats?.map((s: any) => s.coordinates.x) ?? [
      0,
    ])
  );
  const maxCol = Math.max(
    ...($SeatMapData?.decks?.[0]?.seats?.map((s: any) => s.coordinates.y) ?? [
      0,
    ])
  );
  // 3. Initialize grid
