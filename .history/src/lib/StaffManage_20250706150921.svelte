...
<script lang="ts">
  ...
  import { Announcements } from "./Store/announce";
  ...
  async function handleSearch() {
    try {
      ...
      profile_err = {
        message: "Hit Dead End When Fetching User",
        type: "ERROR",
      };
    }
  }
...
<div class="container">
  <div class="sidebar">
    ...
    <div style="margin-top: 1rem">
      ...
      <p>Login Time: {selectedUser.login_time}</p>
      <p>
        Announcements Made: {$Announcements.filter(
          (a) => a.message_author === selectedUser?.username
        ).length}
      </p>
      <p>
        <!-- Schedules Made: {$schFlights.filter(
          (f) =>  === selectedUser?.username
        ).length} -->
      </p>
    </div>

    <button onclick={() => (showBanOverlay = true)}>Ban Staff</button>
    <button onclick={() => (showScheduleOverlay = true)}
      >View Scheduled Flights</button
    >
    <button onclick={() => (showAnnouncementsOverlay = true)}
