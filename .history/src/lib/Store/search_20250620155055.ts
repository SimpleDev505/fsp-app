...
export const currentFlights = writable<StateVector[]>([]);

export const isSearchValid = derived(Current_SearchFilter, ($filter) => {
    return $filter.timings.length > 0 &&
        $filter.timings.every((timing) =>
            timing.from && timing.to && timing.dep
        );
});

export const totalPassengers = derived(Current_SearchFilter, ($filter) => {
    return $filter.passengers.adult + $filter.passengers.child +
        $filter.passengers.infant;
});
