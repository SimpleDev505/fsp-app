...
const dt = targetDate.toISOString().split("T")[0]; // format: YYYY-MM-DD
const WEATHER_API_URL = `https://api.weatherapi.com/v1/future.json?key=${
...
export async function fetchWeather() {
  try {
    ...
    const info = {
      temperatureCelsius: forecast.avgtemp_c,
      weatherType: forecast.condition.text,
      duration: `Forecast for ${dt}`,
    };
