...
import { GetUserData, LoginData, UpdateUserData } from "./login";
...
export async function bookTicket() {
  ...
  try {
    ...
    const login_detail = get(LoginData);
    if (login_detail && login_detail.id) {
      //Update Ticket Count
      let prev_ticket_count = await GetUserData(
        login_detail.id,
        "ticket_booked"
      );
      prev_ticket_count += 1;
      UpdateUserData(
        login_detail.id,
        "ticket_booked",
        prev_ticket_count as Number
      );
      //Update Recent Place
      UpdateUserData(
        login_detail.id,
        "recent_place",
        _selectedFlight?.to.iataCode as String
      );
      //Update Amount Saved
      if (_selectedFlight) {
        const total = _selectedFlight?.price.total as unknown as number;
        const price = _selectedFlight?.price.base as unknown as number;
        const discount: Number = total - price;
        UpdateUserData(login_detail.id, "recent_place", discount);
      }
    }
  } catch (error) {
...
export async function fetchTicketAndOthers({
  ...
  try {
    ...
    if (res.status === 404) {
      ...
      // seatMapData.set([]);
      // otherPassengersOnSameFlight.set([]);
      return false;
    }
    setTimeout(() => {
      fetchingTicket.set(false);
