...
export const LoadingType = writable<"ANN" | "HISTORY" | "WEATHER" | null>(null);
...
export async function fetchWeather() {
  try {
    ...
    if (forecastList) {
      const load_forecastlist =
        data.forecast?.forecastday?.map((day: any) => ({
          date: "loading",
          temperatureCelsius: "loading",
          weatherType: "loading",
        })) ?? [];
      weatherInfo.set({ forecast: load_forecastlist });
      console.log("Loading ...");
      gsap.fromTo(
        ".weather-card",
        {
