<script lang="ts">
  ...
  const API_SEC = import.meta.env.VITE_CLIENTSEC;
  const API_HOST = import.meta.env.VITE_CLIENTID;
  async function fetchFlights() {
    try {
      // 1. Get current search filter
      const searchFilter: SearchFilter = get(Current_SearchFilter);

      // Optional: Log or validate input
      console.log("Using search filter:", searchFilter);
      ...
      // 3. Fetch flight data (no location filters available in this endpoint)
      ...
      // 4. Save to store
      FlightSearchResults.set(flightRes.data);
      console.log("Raw flight data:", flightRes.data);

      // Optional: filter flight data manually here based on `searchFilter`
    } catch (err) {
