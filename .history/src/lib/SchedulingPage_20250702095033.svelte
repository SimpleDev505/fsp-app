<script>
  ...
  const uniqueAmenitiesByDescriptionOnly = $derived(
    (
      selectedflight?.travelerPricings[0].fareDetailsBySegment[0].amenities ||
      []
    ).filter(
      (amenity, index, self) =>
        index === self.findIndex((a) => a.description === amenity.description)
    )
  );
  onMount(() => {
    ...
    <div class="sched-2-view">
      {#if selectedflight}
        ...
        <span class="sched-2-label">
          <span>Aircraft</span>
          <span>
            {#each uniqueAmenitiesByDescriptionOnly as amenity}
