...
<script lang="ts">
  ...
  function showTooltip(event: MouseEvent, content: string) {
    ...
    if (svgElement && circle) {
      ...
      // Calculate tooltip position - center horizontally on the point
      tooltipX = svgRect.left + cx * scaleX - 85; // Subtract half tooltip width for centering
      tooltipY = svgRect.top + cy * scaleY - 40; // 40px offset above the point
    }
  ...
  function animateChart(chartId: string, color: string) {
    ...
    //   "#" + chartId + " rect",
    //   { scale: 0 },
    //   { scale: 1.5, duration: 0.8, ease: "elastic.inOut", stagger: 0.35 },
    //   "-=0.5"
    // );
    // Animate lines
    tl.to(
              ...
              {#each chartPoints as point, i}
                <circle
                  ...
                  onmouseenter={(e) =>
                    showTooltip(
                      e,
                      `${point.date} - ${point.count} announcement${point.count !== 1 ? "s" : ""}`
                    )}
              ...
              {#each chartPoints as point, i}
                <circle
                  ...
                  onmouseenter={(e) =>
                    showTooltip(
                      e,
                      `${getHourLabel(point.hour)} - ${point.count} flight${point.count !== 1 ? "s" : ""}`
                    )}
                  onmouseleave={hideTooltip}
                />
              {/each}
