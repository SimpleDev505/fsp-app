<script lang="ts">
  ...
  // Current month and year for display (starting with June 2025)
  ...
  // Array of days for rendering (explicitly typed as (number | null)[])
  let days: (number | null)[] = $derived(() => {
    let dayArray: (number | null)[] = [];
  ...
  function handleOutsideClick(event: MouseEvent) {
    ...
    if (target && !target.closest(".datepicker-container")) {
      showCalendar = false;
    }
  }
      ...
      <div class="calendar-grid">
        ...
        {#each days as day}
          {#if day !== null}
            <div
              class="day"
              class:selected={isDayInRange(day)}
              on:mousedown={() => selectDate(day)}
            >
              {day}
            </div>
          {:else}
