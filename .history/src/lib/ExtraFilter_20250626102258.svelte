<script lang="ts">
  ...
  let EnableExtraFilter = $state(false);
  ...
  const FILTER_CONFIG = {
    ...
    airlines: [
      ...
      {
        value: "Qatar Airways" as AirlinesType,
        label: "Qatar Airways",
        type: "checkbox",
        active: true,
      },
  ...
  function debugLog(filterName: string, value: any) {
    ...
    if (!EnableExtraFilter) {
      EnableExtraFilter = true;
      ExtraFiltered_Results.set($FlightSearchResults);
    }
    const rawData = $FlightSearchResults?.data ?? [];
    const dictionaries = $FlightSearchResults?.dictionaries ?? {
      carriers: {},
      aircraft: {},
    };
    ...
    const filteredData = rawData.filter((flight) => {
      ...
      const totalStops = flight.itineraries[0]?.segments.length;
      const departureTime = new Date(
        firstSegment?.departure.at ?? ""
      ).getHours();
      const durationHours = parseInt(
        flight.itineraries[0]?.duration?.match(/\d+/)?.[0] ?? "0"
      );
      const airlineName =
        dictionaries.carriers?.[firstSegment?.carrierCode.trim()] ?? "";
      console.log("Name : ", airlineName);
      // Filters
      const isPopularMatch =
        filters.popularfilters === "none" ||
        (filters.popularfilters === "1" && totalStops === 0) ||
        (filters.popularfilters === "2" && totalStops >= 2) ||
        filters.popularfilters === airlineName;

      const isPriceMatch = parseFloat(flight.price.total) <= filters.pricerange;

      const isStopsMatch =
        filters.stops === "none" ||
        (filters.stops === "1" && totalStops === 1) ||
        (filters.stops === "2" && totalStops >= 2);

      const isDepartTimeMatch = (() => {
        if (filters.departime === "Before 6Am") return departureTime < 6;
        if (filters.departime === "6Am to 12Pm")
          return departureTime >= 6 && departureTime < 12;
        if (filters.departime === "12Pm to 6Pm")
          return departureTime >= 12 && departureTime < 18;
        if (filters.departime === "After 6Pm") return departureTime >= 18;
        return true;
      })();

      const isDurationMatch = durationHours <= filters.duration_hrs;

      const isAirlineMatch =
        filters.airlines === "none" ||
        filters.airlines.toUpperCase().trim() === airlineName.trim();
      ...
      return (
        (isDurationMatch || isAirlineMatch || isPopularMatch) &&
        (isPriceMatch || isStopsMatch || isDepartTimeMatch)
      );
    });
    ...
    console.log("FilterData : ", $ExtraFiltered_Results);
  }
  ...
  function updateFilter(filterKey: keyof typeof DEFAULT_FILTERS, value: any) {
    ...
    //Enable extra Filter once and disable on clear
...
<style>
  ...
  .filter-option input[type="radio"] {
    ...
    width: 1em;
    height: 1em;
    border: 0.15em solid var(--grey-dark);
  }
