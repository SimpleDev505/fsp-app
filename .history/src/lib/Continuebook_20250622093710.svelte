            ...
            <div class="baggage-grid">
              {#each Object.entries(baggagecount) as [key, value]}
                ...
                <div class="baggage-item">
                  <div class="weight-label">{typedKey.toUpperCase()}</div>
                  <button on:click={() => decrement(typedKey)}>−</button>
                  <span>{value}</span>
                  <button on:click={() => increment(typedKey)}>+</button>
                </div>
              {/each}
            </div>
          </div>
