...
<script lang="ts">
  import { $state, $effect } from "@maverick-js/signals/svelte";
  import { schFlights } from "$lib/stores/scheduledFlights";
  import { allAnnouncements } from "$lib/stores/announcements";
  import { userList } from "$lib/stores/userList";
  import {
    fetchUserByName,
    fetchAnnouncementsByAuthor,
  } from "$lib/functions/user-api";
  ...
  async function handleSearch() {
    try {
      const user = await fetchUserByName(searchQuery);
      if (user) {
        selectedUser.set(user);
      } else {
        profile_err.set({ message: "User not found", type: "ERROR" });
      }
    } catch (err) {
      profile_err.set({ message: err.message, type: "ERROR" });
    }
  }
</script>
