<script lang="ts">
  ...
  function updateSearchDates() {
    ...
    if (!filter || !filter.timings || filter.timings.length === 0) return;

    const depFormatted = startDate ? formatDate(startDate) : "";
    const returnFormatted = endDate && pickupenddate ? formatDate(endDate) : "";

    filter.timings[0].dep = depFormatted;
    filter.timings[0].return = pickupenddate ? returnFormatted : undefined;

    Current_SearchFilter.set({ ...filter });

    console.log("Filtered -> ", $Current_SearchFilter);
  }
