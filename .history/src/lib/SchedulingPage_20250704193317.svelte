...
<div class="sched-wrapper">
 ...
 padding: 10px; color: white; border-radius: 3px; text-align: center;"
        ...
        <div class="sched-2-tools">
          <button onclick={() => (OpenSchedulingOverlay = true)}>Cancel</button>
          <button onclick={() => (OpenSchedulingOverlay = true)}>Delay</button>
          <button onclick={() => (OpenSchedulingOverlay = true)}
            >Reschedule</button
          >
        </div>
      {:else}
    ...
    {#if $FlightSearchResults?.data}
      <RescheduleView
        type={"RESCHEDULED"}
        flight={$FlightSearchResults.data[0]}
        onClose={() => (OpenSchedulingOverlay = false)}
      ></RescheduleView>
    {/if}
  </div>
</div>
