...
<script lang="ts">
  ...
  let expandedFlights = $state(new Set<string>());
  let activeTab = $state(new Map<string, string>());

  function toggleDetails(flightToken: string) {
    if (expandedFlights.has(flightToken)) {
      expandedFlights.delete(flightToken);
    } else {
      expandedFlights.add(flightToken);
      // Set default active tab
      if (!activeTab.has(flightToken)) {
        activeTab.set(flightToken, "flight-info");
      }
    }
    expandedFlights = new Set(expandedFlights);
    activeTab = new Map(activeTab);
  }

  function switchTab(flightToken: string, tabId: string) {
    activeTab.set(flightToken, tabId);
    activeTab = new Map(activeTab);
  }
</script>

<div class="flight-results-container">
  {#each $FlightSearchResults as flightData}
    <div class="flight-data-section">
      <!-- Optional: Display search criteria or metadata -->
      <div class="search-info">
        <span class="trip-type">{flightData.tripType}</span>
        <span class="cabin-class">{flightData.searchCriteria.cabinClass}</span>
        <span class="results-count"
          >{flightData.aggregation.totalCount} flights found</span
        >
      </div>

      <!-- Flight Offers -->
      {#each flightData.flightOffers as flightOffer}
        {@const primaryCarrier =
          flightOffer.segments[0]?.legs[0]?.carriersData[0]}
        {@const flightNumber =
        ...
        {@const totalJourneyMinutes =
          firstDeparture && lastArrival
            ...
            : 0}
        {@const isDetailsVisible = expandedFlights.has(flightOffer.token)}
        {@const currentTab = activeTab.get(flightOffer.token) || "flight-info"}
          ...
          >
            ...
            <div class="journey-overview">
              <span class="departure-time"
                >{formatTime(firstDeparture || "")}</span
              >
              {#if isDetailsVisible}
                <hr
              ...
              <div class="journey-info">
                <span class="duration"
                  >{formatDuration(totalJourneyMinutes)}</span
                >
                <hr
                  style="width:100%; font-weight: bold; color:blue; height:1px;"
                />
          ...
          {#if isDetailsVisible}
            <div class="flight-details">
              <div class="tabs">
                <button
                  class="tab"
                  class:active={currentTab === "flight-info"}
                ...
                <button
                  class="tab"
                  class:active={currentTab === "fare-details"}
                ...
                <button
                  class="tab"
                  class:active={currentTab === "baggage-info"}
                ...
                <button
                  class="tab"
                  class:active={currentTab === "cancellation-rules"}
                  onclick={() =>
              ...
              <div class="tab-content">
                ...
                {:else if currentTab === "fare-details"}
                  ...
                  <div class="tab-content-section">
                    <div class="fare-breakdown">
                      ...
                      <div class="fare-item">
                        ...
                        <span
                          >{formatPrice(flightOffer.priceBreakdown.tax)}</span
                        >
                      </div>
                    ...
                    <div class="fare-rules">
                      ...
                      <ul>
                        <li>
                          Fare is {flightOffer.brandedFareInfo?.fareName ||
                            "Economy"}
                        </li>
                        <li>
                          Cabin class: {flightOffer.brandedFareInfo
                            ?.cabinClass || "Economy"}
                        </li>
                        <li>
                          Seats available: {flightOffer.seatAvailability
                ...
                {:else if currentTab === "baggage-info"}
                    ...
                    <div class="baggage-info">
                      ...
                      {#if flightOffer.brandedFareInfo?.features}
                        {#each flightOffer.brandedFareInfo.features as feature}
                          {#if feature.category === "baggage"}
                            <div class="baggage-section">
                              <h5>{feature.label}</h5>
                              <p>
                                Status: {feature.availability === "INCLUDED"
                                  ? "Included"
                      ...
                      <div class="baggage-note">
                        <p>
                          <strong>Note:</strong> Extra baggage charges will apply
                          for excess weight/pieces
                        </p>
                      </div>
                    </div>
                  </div>
                {:else if currentTab === "cancellation-rules"}
                      ...
                      {:else}
                        <div class="rule-section">
                          ...
                          <ul>
                            ...
                            <li>
                              24+ hours before departure: ₹2,500 + Airline
                              charges
                            </li>
                          </ul>
                        </div>
                        <div class="rule-section">
                          ...
                          <ul>
                            <li>
                              0-2 hours before departure: ₹2,500 + Fare
                              difference + Airline charges
                            </li>
                            <li>
                              2+ hours before departure: ₹2,000 + Fare
                              difference + Airline charges
                            </li>
                          </ul>
                        </div>
