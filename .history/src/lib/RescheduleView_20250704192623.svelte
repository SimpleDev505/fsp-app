<script lang="ts">
    ...
    const validateDelay = (): boolean => {
      ...
      if (hours === 0 && minutes === 0) {
        profile_err = { message: "Delay time must be greater than 0", type: "ERROR" };
        return false;
      }
    ...
    const validateReschedule = (): boolean => {
      ...
      if (newDate <= currentDate) {
        profile_err = { message: "Rescheduled date must be in the future", type: "ERROR" };
        return false;
      }
    ...
    const validateCancel = (): boolean => {
      if (!reason.trim()) {
        profile_err = { message: "Please provide a reason for cancellation", type: "ERROR" };
        return false;
      }
      return true;
    };
    ...
    const handleConfirm = () => {
      ...
      switch (type) {
        case "DELAY":
          ...
          if (isValid) {
            ...
            const originalTime = new Date(flight.departureTime); // Assuming flight has departureTime
            const delayedTime = new Date(originalTime.getTime() + (hours * 60 + minutes) * 60000);
      ...
      if (isValid) {
        ...
        profile_err = { message: `Flight ${type.toLowerCase()} scheduled successfully`, type: "SUCCESS" };
    ...
    <div class="modal-overlay" on:click={handleOutsideClick}>
      <div class="modal-container">
        <div class="modal-header">
          <h2>{type === 'DELAY' ? 'Delay Flight' : type === 'RESCHEDULED' ? 'Reschedule Flight' : 'Cancel Flight'}</h2>
          <button class="close-btn" on:click={closeModal}>×</button>
        </div>
        
        <div class="modal-body">
          {#if profile_err.message}
            <Notifier P_error={profile_err.message} Type={profile_err.type}></Notifier>
          {/if}
              ...
              <div class="time-inputs">
                <div class="time-group">
                  ...
                  <div class="time-control">
                    ...
                    <div class="time-buttons">
                      <button class="time-btn" on:click={() => incrementTime('hours')}>+</button>
                      <button class="time-btn" on:click={() => decrementTime('hours')}>−</button>
                    </div>
                  </div>
                ...
                <div class="time-group">
                  ...
                  <div class="time-control">
                    ...
                    />
                    <div class="time-buttons">
                      <button class="time-btn" on:click={() => incrementTime('minutes')}>+</button>
                      <button class="time-btn" on:click={() => decrementTime('minutes')">−</button>
                    </div>
                  </div>
          ...
          {#if type === 'RESCHEDULED'}
            <div class="reschedule-container">
              <div class="date-group">
                ...
                <DateDropDown icon="calendar_month" pickupenddate={false}></DateDropDown>
              </div>
        ...
        <div class="modal-footer">
          ...
          <button class="confirm-btn" on:click={handleConfirm}>
            {type === 'DELAY' ? 'Announce Delay' : type === 'RESCHEDULED' ? 'Reschedule' : 'Cancel Flight'}
          </button>
        </div>
      </div>
    </div>
  ...
  <style>
    ...
    .delay-container, .reschedule-container, .cancel-container {
      display: flex;
