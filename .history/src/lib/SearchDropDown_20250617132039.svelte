<script lang="ts">
  ...
  // Props: list of Searches
  let { Searches }: { Searches: SearchItem[] } = $props();

  // State: search input value and dropdown visibility
  let searchValue = $state("");
  let showDropdown = $state(false);

  // Derived: filtered Searches based on search input
  let filteredSearches = $derived(
    searchValue
      ? Searches.filter((airport) =>
...
<div class="dropsearch-container">
  ...
  {#if showDropdown}
    <div class="dropsearch-list">
      {#each filteredSearches as airport}
        <div
