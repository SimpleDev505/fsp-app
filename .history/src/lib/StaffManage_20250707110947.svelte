...
<script lang="ts">
  ...
  async function searchStaff() {
    ...
    try {
      ...
      console.log("SEARCH STAFF ", staff);
      if (staff) {
        currentStaff = staff;
        //Announcements
        if (staff.username) {
  ...
  function toggleBanStaff() {
    ...
    if (currentStaff.banned) {
      if (currentStaff && currentStaff.id) {
        UpdateUserData(currentStaff.id, "banned", false);
        UpdateUserData(currentStaff.id, "ban_time", new Date());
      }
      // currentStaff = {
      //   ...currentStaff,
      //   banned: false,
  ...
  function confirmBan() {
    ...
    // };
    if (currentStaff && currentStaff.id) {
      UpdateUserData(currentStaff.id, "banned", true);
      UpdateUserData(currentStaff.id, "ban_time", new Date());
    }
    showBanModal = false;
    banReason = "";
