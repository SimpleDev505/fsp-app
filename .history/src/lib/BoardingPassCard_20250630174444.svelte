<script lang="ts">
  ...
  import { seatGrid } from "./Store/seat";
  ...
  function assignSeatToPassenger(passengerName: string) {
    ...
    if (seats && seats.length > 0) {
      ...
      // if (
      //   Object.keys(seatAssignments).length ===
      //   $currentTicket?.passengers.length
      // ) {
      //   seatAssignments = {};
      //
      // }
...
<div class="boarding-cnt-wrap">
  <div class="boarding-cnt" bind:this={boardingpass_cnt}>
    ...
    <div class="ticket-search">
      ...
      />
      <button class="refresh-btn" onclick={() => ClearBoardingPass}>R</button>
          ...
          {#if ticketdetail && !$fetchingTicket}
            ...
            {#each ticketdetail.passengers as p, pindex}
              <div class="passenger-row">
                ...
                <span
                  >{seatAssignments[p.fullName.trim()] ?? "Not Assigned"}</span
                >
                <span
