<script lang="ts">
  ...
  const handleConfirm = async () => {
    ...
    if (isValid) {
      ...
      await ScheduleFlight(scheduledFlight);
      profile_err = {
  ...
  let updatingScheduling = $state(false);
  async function ScheduleFlight(scheduledFlight: ScheduledFlight) {
    try {
      updatingScheduling = true;
      const response = await fetch(`http://localhost:8080/api/schedule`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(scheduledFlight),
      });
      ...
      profile_err = { message: result.message, type: "SUCCESS" };

      // After successful scheduling, fetch latest schedules
      await LoadLatestScheduledFlights();
    } catch (error) {
      console.error("Error scheduling flight:", error);
      profile_err = {
        message: "Failed to schedule flight. Please try again.",
        type: "ERROR",
      };
    } finally {
      updatingScheduling = false;
    }
  }

  const LoadLatestScheduledFlights = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/schedule/latest`);
      ...
      FlightHistory.set(schedules);
    } catch (error) {
      console.error("Error loading scheduled flights:", error);
      profile_err = {
        message: "Failed to load flight history.",
        type: "ERROR",
      };
    }
  };
