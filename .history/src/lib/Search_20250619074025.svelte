<script lang="ts">
  ...
  async function FetchFlights() {
    ...
    try {
      ...
      console.log("Dep :", dep_iata, "Arr : ", arr_iata);
      if (!dep_iata || !arr_iata) {
        throw new Error(
          `Invalid location label: "${timing.from}" or "${timing.to}"`
        );
      }
      ...
      const options = {
        ...
        params: {
          fromId: "DEL", // IATA code (e.g., Delhi)
          toId: "BLR", // IATA code (e.g., Bangalore)
      ...
      console.log("Flight API Response:", $FlightSearchResults);
    } catch (error) {
