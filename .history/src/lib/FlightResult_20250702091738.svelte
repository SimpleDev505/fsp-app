  ...
  <div class="main-info" onmousedown={() => (showDetails = !showDetails)}>
    ...
    <div class="price-col">
      ...
      {:else if !markedforreview}
        <button class="book-btn" onclick={() => MarkFlight()}>Mark</button>
      {:else}
        <p
          style="background-color: var(--blue-dark); color: white; 
          outline: 2px solid var(--grey-light); width: fit-content; height: fit-content; margin: 0; padding: 5px;"
        >
          Marked
        </p>
      {/if}
