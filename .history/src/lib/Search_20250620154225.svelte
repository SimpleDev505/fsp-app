<script lang="ts">
  ...
  import type { Flight } from "./Store/search";
  ...
  function filterFlights(isReturn: boolean = false): Flight[] {
    const filter = $Current_SearchFilter;
    const depDate = new Date(isReturn ? timing.return! : timing.dep);
    const [startHour, endHour] = filter.extra?.departime
      ? getDepartTimeRange(filter.extra.departime)
      : [0, 24];
    const airlines = filter.extra?.airlines ? [filter.extra.airlines] : [];

    return flights.filter((flight) => {
      const fromIcao = getIcaoFromCity(timing.from);
      const toIcao = getIcaoFromCity(timing.to);
      if (fromIcao && flight.estDepartureAirport !== fromIcao) return false;
      if (toIcao && flight.estArrivalAirport !== toIcao) return false;

      const departure = new Date(flight.firstSeen * 1000);
      const hours = departure.getHours();
      const departureDate = departure.getDate();
      const depDateDay = depDate.getDate();
      if (departureDate !== depDateDay || hours < startHour || hours >= endHour)
        return false;
