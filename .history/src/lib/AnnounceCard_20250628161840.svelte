  ...
  function handleAnnounce() {
    ...
    if (loggeddata && loggeddata.username) {
      ...
      if (
        ...
        const keysWithEmptyStringValues2 = Object.keys(ann_values2).filter(
          (key) => {
            // Use 'keyof MyObject' directly if 'ann_values2' is typed
            const value = ann_values2[key as keyof MyObject];
            return typeof value === "string" && value === "";
          }
        );
        ann_err = `Fill The Required Data${emptyStrings} , Before announcing Message!`;
        setTimeout(() => {
          ann_err = "";
        }, 4000);
        return;
      ...
      ann_err = "Message Sent SucessFully";
      setTimeout(() => {
        ann_err = "";
      }, 2500);
      return;
...
<div class="announcement-container">
  <div class="announcement-card">
    ...
    >
      {#if ann_err}
        <Notifier P_error={ann_err} Type={"ERROR"}></Notifier>
      {/if}
      <button
