          ...
          <div class="search-multi-cnt">
            ...
            {#if AddedFlightTrips === 1}
             ...
             cursor: pointer;"
                onmousedown={() => {
                  AddedFlightTrips += 1;
                  console.log(AddedFlightTrips);
                }}>Add Route +</button
              >
            {/if}
          </div>
          {#each { length: AddedFlightTrips - 1 }, index}
            <div class="search-multi-cnt">
              <SearchDropDown Title="From" items={airports}></SearchDropDown>
              <SearchDropDown Title="To" items={airports}></SearchDropDown>
              <DateDropDown></DateDropDown>
              {console.log("Multi - ", AddedFlightTrips)}
              {#if AddedFlightTrips - 2 === index}
                <button
                  style="font-size: var(--font-sm); white-space: nowrap; background-color: var(--blue);
             color: white; padding: 5px;border-radius:8px;border:none; box-shadow: none;
             cursor: pointer;"
                  onmousedown={() => {
                    AddedFlightTrips += 1;
                    console.log(AddedFlightTrips);
