<script lang="ts">
  ...
  async function FetchFlights() {
    ...
    const timing = filter.timings[0];
    const allResults = [];
    const API_KEY = import.meta.env.VITE_FLIGHT_RAPID_API_KEY;
    const HOST = import.meta.env.VITE_FLIGHT_RAPID_API_HOST;
    for (const timing of filter.timings) {
      const dep_iata = getIATACode(timing.from);
      const arr_iata = getIATACode(timing.to);
      const flight_date = parseCustomDate(timing.dep);

      if (!dep_iata || !arr_iata) {
        console.warn("Skipping invalid route:", timing);
        continue;
      }
      ...
      const params = {
        ...
        returnDate: timing.return ? parseCustomDate(timing.return) : "",
        numberOfAdults: String(filter.passengers.adult),
        numberOfChildren: String(filter.passengers.child),
        numberOfInfants: String(filter.passengers.infant),
        travelClass: filter.class.toUpperCase(),
        currencyCode: filter.currency.toUpperCase(),
      };
      ...
      try {
        ...
        console.error("Error fetching for:", timing, err);
      }
    }

    FlightSearchResults.set(allResults);
    console.log("Fetched total flights:", allResults.length);
  }
</script>
