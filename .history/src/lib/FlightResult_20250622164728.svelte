<script lang="ts">
  ...
  function handleBookNow(flight: any) {
    ...
    const detail: SelectedFlightDetail = {
      ...
      passengers: [
        {
          info: {
            travelerType: flight.travelerPricings[0]?.travelerType ?? "ADULT",
            fullName: "",
            gender: "M",
            passportNumber: "",
            age: 0,
          },
          count: 1, // Default, can be overridden later during group form filling
          fareOption: flight.travelerPricings[0]?.fareOption ?? "STANDARD",
          brandedFare:
            flight.travelerPricings[0]?.fareDetailsBySegment?.[0]
              ?.brandedFare ?? "",
          cabinClass:
...
<div class="flight-card">
  <div class="main-info" on:click={() => (showDetails = !showDetails)}>
    ...
    border-radius: 1px; padding-top: 1px; padding-bottom: 1px;
    color: var(--yellow); font-weight: bold; font-size: var(--font-s);"
          >Meals Available</span
        >
      {/if}
      <span
        style="padding: 5px; outline: 2px solid var(--grey-light);
      border-radius: 10px; padding-top: 1px; padding-bottom: 1px;
      color: var(--red); font-weight: bold;"
        >Seats {flight.numberOfBookableSeats}</span
      >
      <small class="discount">Discount Applied</small>

      <button class="book-btn" on:click={() => handleBookNow(flight)}
        >BOOK NOW</button
      >
    </div>
  </div>
