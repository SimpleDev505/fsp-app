<script lang="ts">
  ...
  async function FetchFlights() {
    ...
    try {
      ...
      const response_full =
        response.data?.results?.slice(0, 15) || response.data?.slice(0, 15);
      FlightSearchResults.set(response_full);
      console.log("🔍 Multi-Stop Flight Search Response:", response_full);
    } catch (err) {
