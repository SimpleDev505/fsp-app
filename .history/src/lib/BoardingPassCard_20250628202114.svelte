<script lang="ts">
  ...
  let ticketdetail_loading = $state(false);
  ...
  async function FetchSeatMapData() {
    ticketdetail_loading = true;
    setTimeout(() => {
      ticketdetail_loading = false;
      if (ticketdetail && !ticketdetail_loading) {
  ...
  <div class="boarding-cnt">
    ...
    <div class="ticket-detail">
      ...
      {#if ticketdetail && !ticketdetail_loading}
        ...
        <span>Status</span>
      {:else if !ticketdetail_loading && !ticketdetail}
        <p style="width: 100%; text-align: center;">
          Search For Ticket To Obtain Information About It!
        </p>
      {:else if ticketdetail_loading}
        <DataLoader strokeWidth={3} infinite duration={4000}></DataLoader>
      {/if}
    </div>
    <div class="seat-assgn-section">
      <h4 style="background-color: var(--blue-dark); width: 100%;">Seat Map</h4>
      {#if !ticketdetail_loading && seatmap}
        <div class="seat-grid">
