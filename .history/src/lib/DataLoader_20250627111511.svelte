  ...
  onMount(() => {
    ...
    if (infinite) {
      ...
      tl.to(
        {},
        {
          duration: duration / 1000, // Use duration for one cycle
          ease: "none", // Linear ease for continuous loop
...
<style>
  ...
  .progress-stroke {
    /* Keep transition for smooth updates, even in infinite mode */
    transition: stroke-dashoffset 0.1s ease-out;
  }

  /* Only apply 'complete' styles if not in infinite mode */
  .progress-circle.complete:not([class*="infinite"]) .progress-stroke {
    stroke: #08e200;
  }
