<script lang="ts">
  ...
  async function fetchFlights() {
    try {
      ...
      if (!enable_api) {
        SearchLoading = true;
        const savedResponse = await axios.get("/flightoffers.json");
        ...
        setTimeout(() => {
          ...
          FlightSearchResults.set({
            data: savedResponse.data.data,
            dictionaries: savedResponse.data.dictionaries,
          });
          ExtraFiltered_Results.set({
            data: savedResponse.data.data,
            dictionaries: savedResponse.data.dictionaries,
          });
          console.log("Flight offers:", $FlightSearchResults?.data);
          Searched.set(true);
          return;
        }, 4000);
        return;
