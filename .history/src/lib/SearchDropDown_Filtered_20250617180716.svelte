<script lang="ts">
  ...
  function handleSelect(airport: SearchItem) {
    ...
    //   };
    // });
    const parseFromToString = (combinedString :string) => {
  // Match pattern: "something to something"
  const match = combinedString.match(/^(.*?)\s+to\s+(.*)$/i);
    ...
    Current_SearchFilter.update((current) => {
      ...
      return {
        ...
        timings: current.timings.map((timing) => ({
          ...
          to: timing.to !== selectedLabel ? selectedLabel : ,
        })),
      };
    });
