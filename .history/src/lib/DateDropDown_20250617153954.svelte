<script lang="ts">
  ...
  let firstMonthDays = $derived(getDaysForMonth(firstMonth, firstYear));
  let secondMonthDays = $derived(getDaysForMonth(secondMonth, secondYear));
  ...
  // Format selected dates for display (simplified using Svelte Runes)
  ...
  {#if showCalendar}
    <div class="calendar">
      <div class="calendar-header">
        <button onclick={prevMonths}>←</button>
        <span
          >{months[firstMonth]}
            ...
            {#each firstMonthDays as day (day)}
              {#if day !== null}
                <div
                  ...
                  role="button"
                  tabindex="0"
                  onmousedown={() => selectDate(day, firstMonth, firstYear)}
                >
                  {day}
                </div>
              {:else}
            ...
            {#each secondMonthDays as day (day)}
              {#if day !== null}
                <div
                  ...
                  role="button"
                  tabindex="0"
                  onmousedown={() => selectDate(day, secondMonth, secondYear)}
                >
                  {day}
                </div>
              {:else}
