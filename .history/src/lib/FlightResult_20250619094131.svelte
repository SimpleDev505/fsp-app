...
<script lang="ts">
    import type { FlightOffer } from './types'; // Adjust the import path as needed
    ...
    function formatMoney(money: { currencyCode: string; units: number; nanos: number }) {
    ...
    function getLayoverDuration(legs: { departureTime: string; arrivalTime: string }[]) {
      let totalLayover = 0;
  ...
  <div class="flight-card" on:click={() => (isRevealed = !isRevealed)}>
    ...
    <div class="summary">
      <div class="airline-info">
        <img src={offer.segments[0].legs[0].carriersData[0].logo} alt="Airline Logo" class="logo" />
        <span>{offer.segments[0].legs[0].carriersData[0].name} {offer.segments[0].legs[0].flightInfo.flightNumber}</span>
      </div>
      ...
      <div class="route">
        ...
        <span>{offer.segments.length > 1 ? `${offer.segments.length - 1} stop` : 'Direct'}</span>
      ...
      <div class="details">
          ...
          {#each offer.segments as segment, index}
            <div class="leg">
              <div class="route-info">
                <span>{segment.departureAirport.cityName} ({segment.departureAirport.code})</span>
              ...
              <div class="route-info">
                <span>{segment.arrivalAirport.cityName} ({segment.arrivalAirport.code})</span>
        ...
        <div class="baggage-info">
          ...
          {#each offer.includedProducts.segments[0] as baggage}
            <p>
              {baggage.luggageType}: {baggage.maxPiece} piece(s), Max Weight: {baggage.maxWeightPerPiece || baggage.maxTotalWeight} {baggage.massUnit}
            </p>
          {/each}
        </div>
