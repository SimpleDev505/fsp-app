...
<script lang="ts">
  ...
  const announcementChartData = $derived(() => {
  ...
  const flightScheduleChartData = $derived(() => {
  ...
  <div class="search-section">
    <div class="search-box">
      <input
        ...
        class="search-input"
        on:keydown={(e) => e.key === "Enter" && searchStaff()}
      />
        ...
        <div class="chart-wrapper">
          {#if announcementChartData.length > 0}
            <svg viewBox="0 0 300 150" class="chart-svg">
              {#each createSimpleChart(announcementChartData, "#3b82f6", "announcements") as point, i}
                <circle cx={point.x} cy={point.y} r="4" fill="#3b82f6" />
                {#if i < announcementChartData.length - 1}
                  <line
                    x1={point.x}
                    y1={point.y}
                    x2={createSimpleChart(
                      announcementChartData,
                      "#3b82f6",
                      "announcements"
                    )[i + 1].x}
                    y2={createSimpleChart(
                      announcementChartData,
                      "#3b82f6",
                      "announcements"
                    )[i + 1].y}
      ...
      <div class="chart-container">
        ...
        <div class="chart-wrapper">
          {#if flightScheduleChartData.length > 0}
            <svg viewBox="0 0 300 150" class="chart-svg">
              {#each createSimpleChart(flightScheduleChartData, "#ef4444", "flights") as point, i}
                <circle cx={point.x} cy={point.y} r="4" fill="#ef4444" />
                {#if i < flightScheduleChartData.length - 1}
                  <line
                    x1={point.x}
                    y1={point.y}
                    x2={createSimpleChart(
