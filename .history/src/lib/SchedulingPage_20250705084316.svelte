<script lang="ts">
  ...
  let flightresults = $derived(get(FlightSearchResults));
  let OpenSchedulingOverlay: {
    type: "DELAY" | "RESCHEDULED" | "CANCELED";
    open: boolean;
  } = $state({ type: "RESCHEDULED", open: false });
...
<div class="sched-wrapper">
 ...
 padding: 10px; color: white; border-radius: 3px; text-align: center;"
    ...
    <div class="sched-results-cnt">
      <div class="sched-results-viewer">
        {#if flightresults}
          {#each flightresults.data as fres}
 ...
 padding: 10px; color: white; border-radius: 3px; text-align: center;"
        ...
        <div class="sched-2-tools">
          <button
            onclick={() =>
              (OpenSchedulingOverlay = { type: "CANCELED", open: true })}
            >Cancel</button
          >
          <button
            onclick={() =>
              (OpenSchedulingOverlay = { type: "DELAY", open: true })}
            >Delay</button
          >
          <button
            onclick={() =>
              (OpenSchedulingOverlay = { type: "RESCHEDULED", open: true })}
            >Reschedule</button
          >
        </div>
      {:else}
    ...
    {#if OpenSchedulingOverlay.open && $FlightSearchResults?.data}
      <RescheduleView
        type={OpenSchedulingOverlay.type}
        flight={$FlightSearchResults.data[0]}
        onClose={() => (OpenSchedulingOverlay.open = false)}
      ></RescheduleView>
    {/if}
  </div>
</div>
