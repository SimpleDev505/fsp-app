<script lang="ts">
  ...
  function getIataFromCity(city: string): string | null {
    const airport = airports.find((a) => a.label === city);
    return airport ? airport.value : null;
  }
  function getIcaoFromIata(iata: string): string | null {
    ...
    return iataToIcao[iata.toUpperCase()] || null;
  }
  function getIcaoFromCity(city: string): string | null {
    const iata = getIataFromCity(city);
    return iata ? getIcaoFromIata(iata) : null;
  }
  function getDepartTimeRange(departime: string): [number, number] {
    switch (departime) {
      case "Before 6Am":
        return [0, 6];
      case "6Am to 12Pm":
  ...
  function parseDate(dateStr: string): number {
    ...
    const date = new Date(`20${year}`, monthMap[month], parseInt(day));
  ...
  async function FetchFlights() {
    ...
    // const { timings, passengers, currency, class: travelClass } = filter;

    // try {
    //   const legs = timings.map((timing) => {
    //     const from = getIATACode(timing.from);
    //     const to = getIATACode(timing.to);
    //     const date = parseCustomDate(timing.dep);

    //     if (!from || !to)
    //       throw new Error(`Invalid location: ${timing.from}, ${timing.to}`);
    ...
    //       children:
    //         passengers.child > 0
    //           ? Array(passengers.child).fill("8").join(",")
    //           : "",
    //       sort: "BEST",
    ...
    //     },
    //     headers: {
    //       "x-rapidapi-key": import.meta.env.VITE_FLIGHT_RAPID_API_KEY,
    //       "x-rapidapi-host": import.meta.env.VITE_FLIGHT_RAPID_API_HOST,
    //     },
    //   };

    //   const response = await axios.request(options);
    //   FlightSearchResults.set(response.data);
    //   console.log(
    //     "Multi-Stop Flight Search Response:",
    //     $FlightSearchResults
    //   );
    // } catch (err) {
    //   console.error("Multi-stop flight search failed:", err);
    // }
    const filter = get(Current_SearchFilter);

    if (!filter) {
      console.error("No search filter provided");
      return;
    }
    ...
    try {
      let legs: { fromId: string; toId: string; date: string }[] = [];

      if (triptype === "One Way") {
        const one = timings[0];
        legs.push({
          fromId: `${getIATACode(one.from)}.AIRPORT`,
          toId: `${getIATACode(one.to)}.AIRPORT`,
          date: parseCustomDate(one.dep),
        });
      } else if (triptype === "Round Trip") {
        const round = timings[0];
        legs.push({
          fromId: `${getIATACode(round.from)}.AIRPORT`,
          toId: `${getIATACode(round.to)}.AIRPORT`,
          date: parseCustomDate(round.dep),
        });
        legs.push({
          fromId: `${getIATACode(round.to)}.AIRPORT`,
          toId: `${getIATACode(round.from)}.AIRPORT`,
          date: parseCustomDate(round.return ?? ""),
        });
      } else if (triptype === "Multi City") {
        legs = timings.map((timing) => ({
          fromId: `${getIATACode(timing.from)}.AIRPORT`,
          toId: `${getIATACode(timing.to)}.AIRPORT`,
          date: parseCustomDate(timing.dep), // Only use dep; ignore return
        }));
      }

      const childrenAges =
        passengers.child > 0 ? Array(passengers.child).fill("8").join(",") : "";
      ...
      // if (!data || !Array.isArray(data.flightOffers)) {
      //   throw new Error("Invalid API response format: expected flightOffers[]");
      // }

      FlightSearchResults.set(data);
      Searched.set(true);
      console.log(
        "Flight search successful:",
        $FlightSearchResults?.flightOffers ?? $FlightSearchResults
      );
    } catch (err) {
