<script lang="ts">
  ...
  function handleAnnounce() {
    ...
    if (loggeddata && loggeddata.username) {
      ...
      const ann_keys = Object.keys($announcementStore);
      if (
        ann_values.some((value) => typeof value === "string" && value === "")
      ) {
        let ann_v: string[] = ann_values.filter(
          (value) => typeof value === "string" && value === ""
        );
        let ann_v_k: string = ann_keys[ann_v];
        ann_err = `Fill The Required Data${ann_v} , Before announcing Message!`;
        setTimeout(() => {
          ann_err = "";
        }, 4000);
        return;
