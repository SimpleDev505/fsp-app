  ...
  $effect(() => {
    if (assgn_err.message && boardingpass_cnt) {
      gsap.fromTo(
        ...
        { y: 10, stagger: 0.4, filter: "grayscale(100%)" },
        {
          y: 0,
          duration: 0.3,
          stagger: 0.1,
    ...
    <div class="boarding-cnt" bind:this={boardingpass_cnt}>
      ...
      <div class="ticket-search">
        ...
        <button
          ...
          class="refresh-btn"
          onclick={() => ClearBoardingPass()}>R</button
        >
          ...
          <div class="assign-panel">
            ...
            <button
              style="background-color: var(--blue);
               ...
               font-size: 14px;
               transition: all 0.2s;"
              onclick={finalizeAssignments}>Assign Seat(s)</button
            >
          </div>
        </div>
      </div>
  ...
  {:else if $GenerateBoardingPass_State === "Loading Pass"}
    ...
    >
      <DataLoader
        ...
        width={20}
        strokeWidth={4}
        onFinish={() => ($GenerateBoardingPass_State = "Gen Pass")}
      ></DataLoader>
    </div>
  {:else if $GenerateBoardingPass_State === "Gen Pass"}
    ...
    >
      ...
      >
        ...
        border-radius: 4px;
        font-size: 14px;
        transition: all 0.2s;">Generate Pass</button
        >
        <!-- onclick={() => {
          fetchTicketAndOthers({
            ticketNumber: ticketinput,
      ...
      <GenPass ticket={ticketdetail} bind:this={genpass_cnt}></GenPass>
    </div>
  {/if}
