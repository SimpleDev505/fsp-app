<script lang="ts">
  ...
  let ExpandLoginDetail = $state(false);
  ...
  function handlenavMouseOver(set: boolean) {
    ExpandLoginDetail = set;
  }
  onMount(() => {
  ...
  function scrollToContact() {
    const target = document.getElementById("Contact-View");
...
<div class="nav-cnt {nav_sticky ? 'nav-sticky' : 'nav-nonsticky'}">
  ...
  <ul class="navlinks">
    <li class="navitem">
      <button id="home-link" onclick={navClicked}>
        <span
        ...
        <span class="link-content"
          >{#if $CurrentAppPage === "STAFF DASHBOARD" || $CurrentAppPage === "ADMIN DASHBOARD"}
            Dashboard
          {:else}
            Home
          {/if}</span
    ...
    <li class="navitem">
      <button id="about-link" onclick={navClicked}>
        <span
          class="material-symbols-outlined"
          style="font-size: var(--font-sm);"
        >
        ...
        <span class="link-content">About </span>
      </button>
    </li>
    {#if !$Searched && $CurrentAppPage === "HOME"}
      <li class="navitem">
        <button id="contact-link" onclick={navClicked}>
          <span
            style="display: flex; justify-content: center; align-items: center;"
          >
            <span
              class="material-symbols-outlined"
              style="font-size: var(--font-sm); margin-right: 4px;"
            >
              call
            </span>
    ...
    {#if LoggedInVisible}
      <li class="navitem">
        <button
          id="signup_login-link"
          onmouseenter={() => handlenavMouseOver(true)}
          onclick={navClicked}
        >
          {#if !$UserLogged}
            SignUp /Login
          {:else}
            ...
            >
              <span
                class="material-symbols-outlined"
                style="font-size: var(--font-lg); margin-right: 4px;"
              >
                account_circle
              </span>
              Profile
            ...
            <img
              ...
              outline: 2px solid var(--grey-light); border-radius: 50%;"
              src={logindetail?.photo ?? ""}
              alt=""
            />
            <!-- {#if ExpandLoginDetail}
              <div
                class="login-detail"
                onmouseleave={() => handlenavMouseOver(false)}
              >
                <p
                  style="grid-column: 1/2/3; width: 100%; text-align: center; box-shadow: none;outline: 2px solid var(--grey-light); background-color: var(--blue-dark);"
                >
                ...
                <p>
                  ...
                  <span
                    class="login-detail-c"
                    style="color: var(--blue); font-weight: normal;"
                    >{logindetail?.username}
                  </span>
                  <span style="color:white;" class="login-detail-arrow">
                    <img
                      class="login-detail-img"
...
<style>
  ...
  .login-detail p:first-child {
    border-radius: 5px;
  }

  .login-detail p:hover {
    background-color: white;
  }
  .login-detail p:nth-child(2) {
    border-top-left-radius: 5px;
    border-top-right-radius: 5px;
  }
  .login-detail p:last-child {
  ...
  :global(.nav-selected .link-content) {
