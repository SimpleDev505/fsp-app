<script lang="ts">
    ...
    import { overlayEditResult } from './stores';
    ...
    function openDialog() {
      inputValue = initialValue;
      overlayEditResult.set(null); // clear previous
      showDialog = true;
    }
  
    function confirm() {
      const result = { property: propertyName, value: inputValue };
      overlayEditResult.set(result);        // ✅ store in writable
      onConfirm();                          // ✅ run prop function
      showDialog = false;
    }
