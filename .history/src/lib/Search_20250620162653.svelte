<script lang="ts">
  ...
  // Use the setToken function to set the token when the component mounts
  useEffect(() => {
    setToken();
  }, []);
  openSkyAPI.interceptors.request.use((config) => {
  ...
  export async function fetchFlights(): Promise<void> {
    ...
    try {
      ...
      for (const timing of searchFilter.timings) {
        ...
        let _from = "";
        airports.map((e) => {
          if (e.label === timing.to) {
            _to = e.value;
          }
          if (e.label === timing.from) {
            _from = e.value;
          }
        });
        console.log(_to, "f", _from);
        const fromICAO = airportToICAO[_from];
        const toICAO = airportToICAO[_to];
      ...
      FlightSearchResults.set(allFlights[0]);
      console.log("Search Result -> ", allFlights);
    } catch (err) {
      const errorMessage =
