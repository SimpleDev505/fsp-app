...
>
  <button
    ...
    onclick={() => (DropDown = true)}>{DropDownTitle}</button
  >
  {#if DropDown}
    <div
      style="position: absolute; top: 100%;width: fit-content; background: white; border: 1px solid var(--grey-light); border-radius: 4px; z-index: 1000; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"
    >
      {#each items as item}
        <div
          role="button"
